...
public class Query{
	...
	private Properties load() throws FileNotFoundException, IOException{
		...
		try{
			properties.load(new FileInputStream("input/twitterQuery.config"));
		}
	...
	private String[] readString(Properties p, String prop){
		...
		if ((aux.equalsIgnoreCase(null) || aux.trim().equalsIgnoreCase(""))){
			...
			if(this.languages != null && prop.equalsIgnoreCase("keywords")){
	...
	public boolean isEmpty(){
		boolean flag;
		if( this.languages == null && this.keywords == null) 
			flag = true;
		else flag = false;
		return flag;
	}
